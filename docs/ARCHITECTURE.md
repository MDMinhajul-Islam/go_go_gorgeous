...
3. **Worker face parsing.** ONNX preprocessing, inference and class argmax run away from the UI thread. One parsing request is allowed at a time and live results are reused between inferences.
...
`TryOn` remains larger than desired. A later refactor should split camera state, coordinate transforms and renderers into typed modules. This release deliberately avoids rewriting the user's existing renderer changes without a complete visual-regression fixture set.

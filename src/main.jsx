...
import './styles.css'
import { createClassMask, FACE_LABELS, FaceParser } from './faceParser'
...
function TryOn({ product: initial, onClose }) {
  ...
  const parserRef = useRef(null), segmentationRef = useRef(null), parsingRef = useRef(false), lastParseRef = useRef(0), maskCacheRef = useRef(new Map())
  ...
  const scheduleParsing=(source,mirror=false,force=false)=>{const now=performance.now();if(parsingRef.current||(!force&&now-lastParseRef.current<320))return;parsingRef.current=true;lastParseRef.current=now;if(!parserRef.current)parserRef.current=new FaceParser();parserRef.current.parse(source,mirror).then(result=>{segmentationRef.current=result;maskCacheRef.current.clear();setAiStatus('Face parsing active')}).catch(()=>setAiStatus('Landmark fallback')).finally(()=>{parsingRef.current=false})}
  const paintSemanticMask=(ctx,classIds,color,opacity,w,h,blend='multiply',feather=1)=>{if(!segmentationRef.current)return false;const key=`${classIds.join('-')}:${color}:${feather}`;let layer=maskCacheRef.current.get(key);if(!layer){const mask=createClassMask(segmentationRef.current,classIds,feather);layer=document.createElement('canvas');layer.width=mask.width;layer.height=mask.height;const layerCtx=layer.getContext('2d');layerCtx.drawImage(mask,0,0);layerCtx.globalCompositeOperation='source-in';layerCtx.fillStyle=color;layerCtx.fillRect(0,0,layer.width,layer.height);maskCacheRef.current.set(key,layer)}ctx.save();ctx.globalCompositeOperation=blend;ctx.globalAlpha=opacity;ctx.drawImage(layer,0,0,w,h);ctx.restore();return true}
  const renderMakeup=(ctx, pts, w, h)=>{
    ...
    if(activeProduct.category==='lipstick'){
      const parsed=paintSemanticMask(ctx,[FACE_LABELS.upperLip,FACE_LABELS.lowerLip],activeShade.c,.82*alpha,w,h,'multiply',1.35)
      if(parsed)paintSemanticMask(ctx,[FACE_LABELS.upperLip,FACE_LABELS.lowerLip],activeShade.c,.2*alpha,w,h,'source-over',.7)
    ...
    if(activeProduct.category==='eyeliner'){
      ctx.globalCompositeOperation='source-over';ctx.strokeStyle=activeShade.c;ctx.fillStyle=activeShade.c;ctx.globalAlpha=Math.min(1,.72+alpha*.28);ctx.lineWidth=Math.min(6,Math.max(2,w*.0038))*(.72+alpha*.28);ctx.lineCap='round';ctx.lineJoin='round';[leftEye,rightEye].forEach((eye,idx)=>{const upper=eye.slice(0,9).map(id=>pts[id]).filter(Boolean);if(upper.length<3)return;ctx.beginPath();ctx.moveTo(upper[0].x*w,upper[0].y*h);for(let i=1;i<upper.length-1;i++){const midX=(upper[i].x+upper[i+1].x)*w/2,midY=(upper[i].y+upper[i+1].y)*h/2;ctx.quadraticCurveTo(upper[i].x*w,upper[i].y*h,midX,midY)}ctx.lineTo(upper.at(-1).x*w,upper.at(-1).y*h);ctx.stroke();const outer=upper[0],neighbor=upper[1];if(outer&&neighbor){const direction=Math.sign(outer.x-neighbor.x)||1,wingX=outer.x+direction*.018,wingY=outer.y-.008;ctx.beginPath();ctx.moveTo(outer.x*w,outer.y*h);ctx.quadraticCurveTo((outer.x+direction*.01)*w,(outer.y-.006)*h,wingX*w,wingY*h);ctx.stroke()}})
    }
    if(activeProduct.category==='concealer'){
      if(!paintSemanticMask(ctx,[FACE_LABELS.skin],activeShade.c,.13*alpha,w,h,'soft-light',2)){ctx.globalCompositeOperation='soft-light';drawPath(ctx,pts,faceOval,w,h);ctx.fillStyle=activeShade.c;ctx.globalAlpha=.18*alpha;ctx.fill()}
      ctx.globalCompositeOperation='source-over';[[33,133,118],[263,362,347]].forEach(([outerId,innerId,cheekId])=>{const outer=pts[outerId],inner=pts[innerId],cheek=pts[cheekId];if(!outer||!inner||!cheek)return;const cx=(outer.x+inner.x)*w/2,cy=((outer.y+inner.y)*.44+cheek.y*.56)*h,rx=Math.max(12,Math.abs(inner.x-outer.x)*w*.68),ry=Math.max(8,Math.abs(cheek.y-(outer.y+inner.y)/2)*h*.8),gradient=ctx.createRadialGradient(cx,cy,0,cx,cy,rx);gradient.addColorStop(0,activeShade.c+'72');gradient.addColorStop(.58,activeShade.c+'38');gradient.addColorStop(1,activeShade.c+'00');ctx.save();ctx.translate(cx,cy);ctx.scale(1,ry/rx);ctx.beginPath();ctx.arc(0,0,rx,0,Math.PI*2);ctx.fillStyle=gradient;ctx.globalAlpha=.7*alpha;ctx.fill();ctx.restore()})
    }
    if(debugMask&&segmentationRef.current){const key='debug';let mask=maskCacheRef.current.get(key);if(!mask){mask=createClassMask(segmentationRef.current,[FACE_LABELS.skin,FACE_LABELS.upperLip,FACE_LABELS.lowerLip,FACE_LABELS.leftEye,FACE_LABELS.rightEye],0);maskCacheRef.current.set(key,mask)}ctx.globalCompositeOperation='source-over';ctx.globalAlpha=.26;ctx.drawImage(mask,0,0,w,h)}
    ctx.restore()
  }
  const ensureModel=async()=>{if(landmarkerRef.current)return landmarkerRef.current;if(landmarkerPromiseRef.current)return landmarkerPromiseRef.current;setLoading(true);landmarkerPromiseRef.current=(async()=>{try{const {FaceLandmarker,FilesetResolver}=await import('@mediapipe/tasks-vision');const vision=await FilesetResolver.forVisionTasks('https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.22-rc.20250304/wasm');const instance=await FaceLandmarker.createFromOptions(vision,{baseOptions:{modelAssetPath:'https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/1/face_landmarker.task',delegate:'GPU'},runningMode:'IMAGE',numFaces:1,minFaceDetectionConfidence:.35,minFacePresenceConfidence:.35,minTrackingConfidence:.35});landmarkerRef.current=instance;runningModeRef.current='IMAGE';return instance}catch(e){landmarkerPromiseRef.current=null;setError('Face model could not load. Check your connection and try again.');throw e}finally{setLoading(false)}})();return landmarkerPromiseRef.current}

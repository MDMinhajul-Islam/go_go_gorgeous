...
function TryOn({ product: initial, onClose }) {
  ...
  const renderMakeup=(ctx, pts, w, h)=>{
    ...
    if(activeProduct.category==='lipstick'){
      const lipMask=()=>{ctx.beginPath();[lipOuter,lipInner].forEach(ring=>{ring.forEach((id,i)=>{const p=pts[id];if(!p)return;(i?ctx.lineTo.bind(ctx):ctx.moveTo.bind(ctx))(p.x*w,p.y*h)});ctx.closePath()})}
      ctx.filter=`blur(${Math.max(.45,w*.00055)}px)`;ctx.globalCompositeOperation='multiply';lipMask();ctx.fillStyle=activeShade.c;ctx.globalAlpha=.9*alpha;ctx.fill('evenodd')
      ctx.globalCompositeOperation='source-over';lipMask();ctx.fillStyle=activeShade.c;ctx.globalAlpha=.2*alpha;ctx.fill('evenodd');ctx.filter='none'
    }
    if(activeProduct.category==='eyeliner'){
      ctx.globalCompositeOperation='source-over';ctx.strokeStyle=activeShade.c;ctx.fillStyle=activeShade.c;ctx.globalAlpha=Math.min(1,.65+alpha*.35);ctx.lineWidth=Math.max(3,w*.009)*(.65+alpha*.35);ctx.lineCap='round';ctx.lineJoin='round';[leftEye,rightEye].forEach((eye,idx)=>{ctx.beginPath();eye.slice(0,9).forEach((id,i)=>{const p=pts[id];(i?ctx.lineTo.bind(ctx):ctx.moveTo.bind(ctx))(p.x*w,p.y*h)});ctx.stroke();const outer=pts[idx?263:33],inner=pts[idx?362:133];if(outer&&inner){const dir=idx?1:-1;ctx.beginPath();ctx.moveTo(outer.x*w,outer.y*h);ctx.lineTo((outer.x+dir*.055)*w,(outer.y-.022)*h);ctx.lineTo((outer.x+dir*.006)*w,(outer.y+.009)*h);ctx.closePath();ctx.fill()}})
    }
    if(activeProduct.category==='concealer'){
      ctx.globalCompositeOperation='soft-light';drawPath(ctx,pts,faceOval,w,h);ctx.fillStyle=activeShade.c;ctx.globalAlpha=.42*alpha;ctx.fill()
      ctx.globalCompositeOperation='source-over';ctx.fillStyle=activeShade.c;ctx.globalAlpha=.22*alpha;ctx.shadowColor=activeShade.c;ctx.shadowBlur=Math.max(16,w*.02);[[33,133,117,118,101,36],[263,362,346,347,330,266]].forEach(region=>{drawPath(ctx,pts,region,w,h);ctx.fill()});ctx.shadowBlur=0
    } ctx.restore()
  }
  ...
  const startCamera=async()=>{cancelAnimationFrame(rafRef.current);streamRef.current?.getTracks().forEach(t=>t.stop());lastLandmarksRef.current=null;setMode('camera');setError('');try{streamRef.current=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:1280}},audio:false});setTimeout(async()=>{const v=videoRef.current;if(!v)return;v.srcObject=streamRef.current;await v.play();const lm=await setLandmarkerMode('VIDEO');let lastVideoTime=-1;const loop=()=>{if(!videoRef.current||!canvasRef.current)return;const c=canvasRef.current,ctx=c.getContext('2d'),v=videoRef.current;if(v.videoWidth){c.width=v.videoWidth;c.height=v.videoHeight;ctx.save();ctx.translate(c.width,0);ctx.scale(-1,1);ctx.drawImage(v,0,0,c.width,c.height);ctx.restore();if(v.currentTime!==lastVideoTime){lastVideoTime=v.currentTime;const res=lm.detectForVideo(v,performance.now());if(res.faceLandmarks?.[0]){const detected=res.faceLandmarks[0].map(p=>({...p,x:1-p.x})),previous=lastLandmarksRef.current;lastLandmarksRef.current=previous?detected.map((p,i)=>{const dx=p.x-previous[i].x,dy=p.y-previous[i].y,motion=Math.sqrt(dx*dx+dy*dy),follow=Math.min(.88,.22+motion*42);return{x:previous[i].x*(1-follow)+p.x*follow,y:previous[i].y*(1-follow)+p.y*follow,z:(previous[i].z||0)*(1-follow)+(p.z||0)*follow}}):detected}}if(lastLandmarksRef.current)renderMakeup(ctx,lastLandmarksRef.current,c.width,c.height)}rafRef.current=requestAnimationFrame(loop)};loop()},80)}catch(e){setError('Camera access was blocked. Allow camera permission or upload a photo.') }}

...
function TryOn({ product: initial, onClose }) {
  ...
  const videoRef = useRef(null), canvasRef = useRef(null), imageRef = useRef(null), landmarkerRef = useRef(null), landmarkerPromiseRef = useRef(null), runningModeRef = useRef('IMAGE'), rafRef = useRef(), streamRef=useRef(), makeupStateRef=useRef()
  ...
  const renderMakeup=(ctx, pts, w, h)=>{
    ...
    ctx.save(); const alpha=activeIntensity/100; ctx.globalCompositeOperation='multiply';
    if(activeProduct.category==='lipstick'){drawPath(ctx,pts,lipOuter,w,h); ctx.fillStyle=activeShade.c;ctx.globalAlpha=.78*alpha;ctx.fill();drawPath(ctx,pts,lipInner,w,h);ctx.globalCompositeOperation='destination-out';ctx.globalAlpha=.8;ctx.fill()}
    if(activeProduct.category==='eyeliner'){ctx.strokeStyle=activeShade.c;ctx.fillStyle=activeShade.c;ctx.lineWidth=Math.max(2,w*.006)*alpha;ctx.lineCap='round';[leftEye,rightEye].forEach((eye,idx)=>{ctx.beginPath();eye.slice(0,9).forEach((id,i)=>{const p=pts[id];(i?ctx.lineTo.bind(ctx):ctx.moveTo.bind(ctx))(p.x*w,p.y*h)});ctx.stroke();const outer=pts[idx?263:33], inner=pts[idx?362:133]; if(outer&&inner){const dir=idx?1:-1;ctx.beginPath();ctx.moveTo(outer.x*w,outer.y*h);ctx.lineTo((outer.x+dir*.045)*w,(outer.y-.018)*h);ctx.lineTo((outer.x+dir*.008)*w,(outer.y+.006)*h);ctx.fill()}})}
    if(activeProduct.category==='concealer'){drawPath(ctx,pts,faceOval,w,h);ctx.clip();const grad=ctx.createRadialGradient(w*.5,h*.45,0,w*.5,h*.45,w*.4);grad.addColorStop(0,activeShade.c+'cc');grad.addColorStop(1,activeShade.c+'00');ctx.fillStyle=grad;ctx.globalAlpha=.22*alpha;ctx.fillRect(0,0,w,h)} ctx.restore()
  }
  const ensureModel=async()=>{if(landmarkerRef.current)return landmarkerRef.current;if(landmarkerPromiseRef.current)return landmarkerPromiseRef.current;setLoading(true);landmarkerPromiseRef.current=(async()=>{try{const {FaceLandmarker,FilesetResolver}=await import('@mediapipe/tasks-vision');const vision=await FilesetResolver.forVisionTasks('https://cdn.jsdelivr.net/npm/@mediapipe/tasks-vision@0.10.22-rc.20250304/wasm');const instance=await FaceLandmarker.createFromOptions(vision,{baseOptions:{modelAssetPath:'https://storage.googleapis.com/mediapipe-models/face_landmarker/face_landmarker/float16/1/face_landmarker.task',delegate:'GPU'},runningMode:'IMAGE',numFaces:1});landmarkerRef.current=instance;runningModeRef.current='IMAGE';return instance}catch(e){landmarkerPromiseRef.current=null;setError('Face model could not load. Check your connection and try again.');throw e}finally{setLoading(false)}})();return landmarkerPromiseRef.current}
  ...
  const startCamera=async()=>{cancelAnimationFrame(rafRef.current);streamRef.current?.getTracks().forEach(t=>t.stop());setMode('camera');setError('');try{streamRef.current=await navigator.mediaDevices.getUserMedia({video:{facingMode:'user',width:{ideal:1280}},audio:false});setTimeout(async()=>{const v=videoRef.current;if(!v)return;v.srcObject=streamRef.current;await v.play();const lm=await setLandmarkerMode('VIDEO');let lastVideoTime=-1;const loop=()=>{if(!videoRef.current||!canvasRef.current)return;const c=canvasRef.current,ctx=c.getContext('2d'),v=videoRef.current;if(v.videoWidth){c.width=v.videoWidth;c.height=v.videoHeight;ctx.save();ctx.translate(c.width,0);ctx.scale(-1,1);ctx.drawImage(v,0,0,c.width,c.height);ctx.restore();if(v.currentTime!==lastVideoTime){lastVideoTime=v.currentTime;const res=lm.detectForVideo(v,performance.now());if(res.faceLandmarks?.[0]){const mirrored=res.faceLandmarks[0].map(p=>({...p,x:1-p.x}));renderMakeup(ctx,mirrored,c.width,c.height)}}}rafRef.current=requestAnimationFrame(loop)};loop()},80)}catch(e){setError('Camera access was blocked. Allow camera permission or upload a photo.') }}
